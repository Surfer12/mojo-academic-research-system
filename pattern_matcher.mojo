...
@value
struct ResearchPattern(Copyable, Movable):
    ...
    fn __init__(inout self, pattern_id: String, pattern_type: String,
               keywords: List[String], weight: Float32 = 1.0, context_window: Int = 50):
    ...
    fn __copyinit__(inout self, existing: Self):
        self.pattern_id = existing.pattern_id
        self.pattern_type = existing.pattern_type
        self.keywords = existing.keywords
        self.weight = existing.weight
        self.context_window = existing.context_window
...
@value
struct PatternMatch(Copyable, Movable):
    var pattern_id: String
    ...
    fn __copyinit__(inout self, existing: Self):
        ...
        self.matched_keywords = existing.matched_keywords
...
@value
struct ResearchSignature(Copyable, Movable):
    ...
    fn __copyinit__(inout self, existing: Self):
        self.author_name = existing.author_name
        self.signature_patterns = existing.signature_patterns
        self.minimum_confidence = existing.minimum_confidence
        self.combination_rules = existing.combination_rules
    ...
    fn copy(self) -> Self:
        var result = ResearchSignature(self.author_name, self.minimum_confidence)
        result.signature_patterns = self.signature_patterns
        result.combination_rules = self.combination_rules
        return result

struct PatternMatcher:
    var signatures: Dict[String, ResearchSignature]
    var pattern_cache: Dict[String, List[PatternMatch]]
    var text_embeddings: Dict[String, List[Float32]]

    fn __init__(inout self):
        self.signatures = Dict[String, ResearchSignature]()
        self.pattern_cache = Dict[String, List[PatternMatch]]()
        self.text_embeddings = Dict[String, List[Float32]]()

    fn create_oates_signature(self) -> ResearchSignature:
    ...
    fn calculate_aggregate_confidence(self, matches: List[PatternMatch],
        ...
        for entry in pattern_scores.items():
            var pattern_id = entry.key
    ...
    fn extract_research_metadata(self, text: String, matches: List[PatternMatch]) -> Dict[String, String]:
        ...
        for entry in domain_indicators.items():
